...
export const emit${ResourceName}Upsert = async (
  ...
  if (!io) {
    resourceLogger.logFlow({
      ...
      step: "error",
      details: { ${resourceName.singular}Id, error: "Socket server not available", type: "single" },
    })
    return
  }
  ...
  if (!row) {
    resourceLogger.logFlow({
      ...
      action: "socket-update",
      step: "error",
      details: { ${resourceName.singular}Id, error: "${ResourceName} not found", type: "single" },
    })
    if (previousStatus) {
  ...
  resourceLogger.logFlow({
    ...
    action: "socket-update",
    step: "success",
    details: { ${resourceName.singular}Id, previousStatus, newStatus, type: "single" },
  })
}
...
export const emit${ResourceName}Remove = async (
  ...
  resourceLogger.logFlow({
    ...
    step: "success",
    details: { ${resourceName.singular}Id, status },
  })
}
