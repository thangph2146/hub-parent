...
import { ORDER_LABELS } from "../constants/messages"
...
interface UseRowActionsOptions {
  ...
  onHardDelete: (row: OrderRow) => void
  onRestore: (row: OrderRow) => void
  ...
  const renderActiveRowActions = useCallback(
    (row: OrderRow) => {
      const actions: RowActionConfig[] = [
        {
          label: ORDER_LABELS.VIEW_DETAIL,
          icon: Eye,
          onSelect: () => router.push(`/admin/orders/${row.id}`),
        },
      ]

      if (canManage) {
        actions.push({
          label: ORDER_LABELS.EDIT,
          icon: Pencil,
          onSelect: () => router.push(`/admin/orders/${row.id}/edit`),
        })
      }
